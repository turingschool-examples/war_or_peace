...
RSpec.describe Turn do
  ...
  it 'executes a war turn' do
    ...
    expect(turn.spoils_of_war).to include([card1, card4, card5, card2, card3, card6])
    turn.award_spoils(winner) #award spoils (if applicable)

    expect(player1.deck.cards).to eq([card2, card5, card8])
    expect(player2.deck.cards).to eq([card1, card4, card5, card2, card3, card6, card7])
  end

  it 'excutes a mutually assure destruction turn' do

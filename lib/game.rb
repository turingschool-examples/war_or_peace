class Game
  ...
  def trigger
    if gets.chomp.upcase == "GO"
      p "it kinda worked"
      game.start
    else p "That is not an accepted response. Please type 'GO' to start the game!"
    end
  end
  ...
  new_deck = [card1, card2, card3, card4, card5, card6, card7, card8, card9, card10, card11, card12, card13, card14, card15, card16, card17, card18, card19, card20, card21, card22, card23, card24, card25, card26, card27, card28, card29, card30, card31, card32, card33, card34, card35, card36, card37, card38, card39, card40, card41, card42, card43, card44, card45, card46, card47, card48, card49, card50, card51, card52]
  deck1 = []
  deck2 = []
  ...
  def shuffle_deal
    26.times do
      deck1 << new_deck.slice(1 + rand(52))
    end

    deck2.concat(new_deck)
  end

...
class Game
  attr_reader :player1, :player2, :player1_deck, :player2_deck
  def initialize()
    @cards = stack_of_cards
    # make decks
    @player1_deck = Deck.new(@cards[0..25])
    @player2_deck = Deck.new(@cards[26..52])

    # make players
    @player1 = Player.new("Aurora", @player1_deck)
    @player2 = Player.new("Megan", @player2_deck)
    @turn_count = 0
  ...
  def stack_of_cards
    ...
    return [card9, card10, card11, card12, card13, card14, card15, card16, card17, card18, card19, card20, card21, card22, card23, card24, card25, card26, card27, card28, card29, card30, card31, card32, card33, card34, card35, card36, card37, card38, card39, card40, card41, card42, card43, card44, card45, card46, card47, card48, card49, card50, card51, card52, card1, card2, card3, card4, card5, card6, card7, card8]
  ...
  def start
    until player1.has_lost? || player2.has_lost? || @turn_count == 1000000
      ...
      turn.award_spoils(winner)
        if type == :basic
          puts "Turn #{@turn_count} ~ #{winner.name} has won 2 more cards!"
        elsif type == :war
          puts "Turn #{@turn_count} ~ #{winner.name} has won 6 more cards!"
        elsif type == :mutually_assured_destruction
          puts "Turn #{@turn_count} ~ Both players destroyed in mutually assured destruction! 6 cards have been removed from play!"
        end
      @turn_count +=1
    end
    ...
    elsif until @turn_count == 1000000

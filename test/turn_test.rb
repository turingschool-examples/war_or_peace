...
#
  ...
  def test_award_spoils
    ...
    assert_equal @player1.deck.cards.length, 5 #[@card2, @card5, @card8, @card1, @card3]
  end
  ...
  def test_war_spoils
    ...
    turn.pile_cards
    turn.award_spoils(winner)
    assert_equal @player1.deck.cards.length,  7
    assert_equal @player2.deck.cards, [@card7]         #card5,  card4 , card3
  end
    ...
    def test_mutual_loss
      ...
      @deck2 = Deck.new([@card1, @card3, @card8, @card6])
      @player1 = Player.new("Megan", @deck1)
      @player2 = Player.new("Aurora", @deck2)
      turn = Turn.new(@player1, @player2)
      turn.type
